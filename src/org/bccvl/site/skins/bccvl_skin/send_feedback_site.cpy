...
send_to_address = 'servicedeskwebform@griffith.edu.au'
...
# Filter all leading % signs from message (but we can't use re here)
lines = []
...
variables = {'sender_from_address' : sender_from_address,
             ...
             'subject'             : subject,
             'message'             : message,
             'CATEGORY'            : 'eResearch services.bccvl',
             'GROUP'               : 'SIR - eResearch Services - bccvl',
             'BUSINESS'            : 'Research technology support',
             'PRIORITY'            : '5',
             'TYPE'                : 'R',
}
...
%GROUP={GROUP}
%BUSINESS={BUSINESS}
%FROM_EMAIL_OVERRIDE={sender_from_address}
%PRIORITY={PRIORITY}
%TYPE={TYPE}
%SUMMARY={subject}
%DESCRIPTION=Client sent feedback for BCCVL

Client: {sender_fullname} {sender_from_address}
URL: {url}
...
""".format(**variables)
    ...
    result = host.send(message, send_to_address, envelope_from,
                       subject='request', charset=encoding)
