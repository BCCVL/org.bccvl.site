...
*/
Faceted.CheckboxTreeWidget = function(wid){
    this.wid = wid;
    ...
    if(this.maxitems){
        this.fieldset.collapsible({
            ...
            elements: 'li:not(.faceted-checkboxtree-item-zerocount)'
        });
    }
...
Faceted.CheckboxTreeWidget.prototype = {
    ...
    criteria_body: function(){
        ...
        widget.selected.each(function(i){
            var span = jQuery('<span class="faceted-checkboxtree-criterion">');
    ...
    count_update: function(data, sortcountable){
        ...
        jQuery(lis).each(function(){
            var li = jQuery(this);
            li.removeClass('faceted-checkboxtree-item-disabled');
            li.removeClass('faceted-checkboxtree-item-zerocount');
            ...
            if(!value){
                li.addClass('faceted-checkboxtree-item-disabled');
                if(context.widget.hasClass('faceted-zero-count-hidden')){
                    li.addClass('faceted-checkboxtree-item-zerocount');
                }
...
Faceted.initializeCheckboxTreeWidget = function(evt){
  jQuery('div.faceted-checkboxtree-widget').each(function(){
      var wid = jQuery(this).attr('id');
      wid = wid.split('_')[0];
      Faceted.Widgets[wid] = new Faceted.CheckboxTreeWidget(wid);
  });
};
...
jQuery(document).ready(function(){
    jQuery(Faceted.Events).bind(
        ...
        Faceted.initializeCheckboxTreeWidget);
});
