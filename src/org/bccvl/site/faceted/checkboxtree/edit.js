FacetedEdit.CheckboxTreeWidget = function(wid){
    var self = this;
    this.wid = wid;
...
FacetedEdit.CheckboxTreeWidget.prototype = {
    count: function(){
...
FacetedEdit.initializeCheckboxTreeWidget = function(){
    jQuery('div.faceted-checkboxtree-widget').each(function(){
        var wid = jQuery(this).attr('id');
        wid = wid.split('_')[0];
        FacetedEdit.Widgets[wid] = new FacetedEdit.CheckboxTreeWidget(wid);
    });
};
...
jQuery(document).ready(function(){
    jQuery(FacetedEdit.Events).bind(
        ...
        FacetedEdit.initializeCheckboxTreeWidget);
});
